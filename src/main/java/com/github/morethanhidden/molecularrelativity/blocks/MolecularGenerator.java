...
import com.github.morethanhidden.molecularrelativity.TileMolecularGenerator;
...
public class MolecularGenerator extends BlockContainer
{
    @SideOnly(Side.CLIENT)
    private IIcon texture_front;
	private boolean texture_toggle;
	...
	public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int par6, float par7, float par8, float par9){
		...
		TileMolecularGenerator te = (TileMolecularGenerator) world.getTileEntity(x, y, z);
		
		if(te != null){
			if(te.initialized != false){
			if(player.isSneaking()) te.energy = 0.00;
			if(player.isSneaking()) te.euzebufferamount = 0;
			player.addChatComponentMessage(new ChatComponentTranslation("I'm Working"));
    ...
    public void registerBlockIcons(IIconRegister iconreg)
    {
        ...
        this.texture_front = iconreg.registerIcon(this.texture_toggle ? "molecularrelativity:MolecularGeneratorActivated" : "molecularrelativity:MolecularGenerator");
    }
	

	@Override
	public TileEntity createNewTileEntity(World world, int metadata) {
		return new TileMolecularGenerator();
	}
}